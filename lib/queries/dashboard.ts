...
export async function ensureListingOwnerRecord() {
  ...
  const slug = `owner-${profile.id.slice(0, 6)}-${Math.floor(Math.random() * 1000)}`;
  const { data, error } = await supabase
    .from('agents')
    // @ts-ignore
    .insert({
      id: profile.id,
      display_name: profile.full_name || 'EstateNova Owner',
      bio: 'Owner-managed listing profile',
      languages: ['English'],
      service_areas: [],
      is_active: true,
      public_slug: slug,
...
export async function getDashboardOverview(): Promise<DashboardOverviewFn> {
  const supabase = await createServerSupabaseClient();

  // B) Recent Listings
  const recentListingsQuery = supabase
    .from('listings')
  ...
  // Listings Counts
  const totalListingsQuery = supabase.from('listings').select('id', { count: 'exact', head: true });
  const publishedListingsQuery = supabase.from('listings').select('id', { count: 'exact', head: true }).eq('status', 'PUBLISHED');
