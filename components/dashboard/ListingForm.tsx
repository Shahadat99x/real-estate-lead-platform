...
export function ListingForm({ listing }: { listing?: FormListing }) {
  ...
  const [state, formAction] = useFormState(initialAction, {});

  if (state?.success && listing?.id) {
    // stay on page
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      {state?.success && <p className="text-sm text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2">Saved.</p>}

      <div className="flex flex-wrap gap-3">
        <SubmitButton label="Save" />
        {listing?.id && (
          <>
            <form action={deleteListingAction} className="inline-flex" onSubmit={(e) => {
            ...
            <form action={setPublishedAction} className="inline-flex">
              <input type="hidden" name="id" value={listing.id} />
